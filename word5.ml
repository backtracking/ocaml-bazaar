...
let () =
  ...
  if !l < !l' then
  let k = m lor m' in
  let r = try PT.find k !words2
          with Not_found -> let r = ref [] in words2 := PT.add k r !words2; r in
  r := (!l @ !l') :: !r) !words) !words;
  printf "%d masks for 2 words@." (PT.cardinal !words2)
...
let () =
  PT.iter 0  (fun m1 l1 ->
  PT.iter m1 (fun m2 l2 ->
  PT.iter (m1 lor m2) (fun m l ->
    incr nbsol;
    List.iter print (!l :: !l1 @ !l2); printf "@."
  ) !words) !words2) !words2;
